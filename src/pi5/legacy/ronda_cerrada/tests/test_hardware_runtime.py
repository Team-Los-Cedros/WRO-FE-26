...
from src.pi5.ronda_cerrada.hardware import EnlacePicoNuevo, FuenteCamara
from src.pi5.ronda_cerrada.ronda_nueva import AplicacionRondaNueva
...
class FuenteCamaraTests(unittest.TestCase):
    ...
    def test_ae_awb_no_soportado_es_advertencia_y_captura_continua(self):
        ...
        with patch.dict(sys.modules, {"picamera2": modulo}), patch(
            "src.pi5.ronda_cerrada.hardware.time.sleep", return_value=None
        ):
...
class WatchdogPicoTests(unittest.TestCase):
    ...
    def test_arranque_envia_freno_hasta_recibir_wd_ok(self):
        ...
        app.enlace = enlace
        with patch(
            "src.pi5.ronda_cerrada.ronda_nueva.time.sleep", return_value=None
        ):
    ...
    def test_firmware_sin_wd_no_puede_armar(self):
        ...
        app.enlace = enlace
        with patch(
            "src.pi5.ronda_cerrada.ronda_nueva.time.monotonic",
