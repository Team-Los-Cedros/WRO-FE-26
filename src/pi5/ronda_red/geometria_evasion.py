...
def cruza_al_lado(x_r, y_r, s_lado, cmd, holgura):
    """¿Sostener `cmd` lleva a su lado un pilar que hoy esta del prohibido?

    `preserva_lado` responde a "¿lo mantengo donde debe estar?" y por eso
    devuelve False para todo comando mientras el pilar esta del lado malo.
    Esta funcion responde a la otra mitad: cual de los arcos ejecutables
    CRUZA el pilar al lado bueno. Es la pregunta que hay que hacerse con
    el SIGUIENTE pilar mientras todavia se rodea el actual.

    DEMOSTRACION. Girando hacia el pilar el centro queda en (s, 0) con
    s del mismo signo que x_r. Si el pilar esta FUERA del anillo barrido

        d = hypot(x_r - s, y_r) >= radio_exterior + RADIO_POSTE + holgura

    el robot pasa por dentro de el. Como d y el anillo no cambian a lo
    largo de la circunferencia, la desigualdad se mantiene todo el arco:
    el pilar no se toca y queda por fuera del giro, o sea del costado
    contrario al centro -- que es el lado bueno, porque el centro esta
    del lado prohibido. La separacion minima vale d - radio_exterior -
    RADIO_POSTE >= holgura.

    Girando hacia el otro lado el pilar se queda donde esta: no cruza.
    """
    u = s_lado * x_r
    if u >= SEMIANCHO + RADIO_POSTE + holgura or y_r <= -geo.VOLADIZO_TRASERO:
        return False        # ya esta de su lado, o ya quedo atras
    radio = radio_de_comando(cmd)
    if radio == float("inf"):
        return False        # recto no cruza a nadie
    centro_x = -radio if cmd > 0 else radio
    if (centro_x * s_lado) > 0:
        return False        # se gira hacia el lado bueno: lo empuja mas lejos
    d = math.hypot(x_r - centro_x, y_r)
    return (d - radio_exterior(radio)) >= RADIO_POSTE + holgura


def margen_cruce(x_r, y_r, s_lado, cmd):
    """Milimetros de sobra con que `cmd` cruza el pilar; negativo si no."""
    radio = radio_de_comando(cmd)
    if radio == float("inf"):
        return -1e4
    centro_x = -radio if cmd > 0 else radio
    if (centro_x * s_lado) > 0:
        return -1e4
    d = math.hypot(x_r - centro_x, y_r)
    return d - radio_exterior(radio) - RADIO_POSTE
