#!/bin/bash
# Ejecutar EN LA PI 5, desde cualquier directorio dentro del clon:
#   bash src/pi5/deploy.sh --dry-run /home/pi/wro_pi5
#   bash src/pi5/deploy.sh /home/pi/wro_pi5
#
...
# existe. Nunca mezcla ni actualiza una instalacion anterior, y NO TOCA NADA
# de src/pi3B: la Pi 3B sigue arrancando igual que antes.
set -euo pipefail
...
Uso: deploy.sh [--dry-run|--preflight] [DESTINO]
  ...
  DESTINO                 por defecto: /home/pi/wro_pi5

El destino debe no existir. Para conservar una instalacion previa, elige otro
nombre (por ejemplo, /home/pi/wro_pi5_20260904).
EOF
}
...
ORIGEN_PI5="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
DESTINO_ENTRADA="${1:-/home/pi/wro_pi5}"
...
if (( ${#FUENTES_ABIERTA[@]} > 0 )); then
    ...
    cp -- "${FUENTES_ABIERTA[@]}" "$STAGING/ronda_abierta/"
fi
if (( ${#FUENTES_HERRAMIENTAS[@]} > 0 )); then
    cp -- "${FUENTES_HERRAMIENTAS[@]}" "$STAGING/herramientas/"
fi
cp -- "$ORIGEN_PI5/requirements.txt" "$STAGING/"
...
echo "[+] ronda de la Pi 5 desplegada como paquete nuevo en $DESTINO"
