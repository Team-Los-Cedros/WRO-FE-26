...
# La red del Hailo corre EN SOMBRA: mira los mismos cuadros y anota lo que ve,
# pero no decide nada. El control sigue siendo el de aqui abajo, intacto. Sirve
# para comparar los dos detectores sobre las mismas corridas antes de cambiar
# nada. Se enciende con WRO_RED=1; si algo falla al cargarla, se queda apagada
# y la ronda corre igual que siempre.
_RED = os.environ.get("WRO_RED") == "1"
if _RED:
    try:
        import vision_red
        _RED = vision_red.arrancar()
    except Exception as _e:
        print("[red] no se pudo cargar (%s): sigue solo el HSV" % _e)
        _RED = False
...
def procesar_frame(frame):
    ...
    try:
        ...
        # Fuera del lock: la red mira el cuadro en su propio hilo y a su ritmo.
        if _RED:
            vision_red.publicar(frame, mejor_color, mejor_cx, mejor_area)
