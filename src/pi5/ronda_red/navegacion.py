...
from dos_pilares import SiguientePilar, preparar_paso, MEJORA_MINIMA_MM
from reproduccion_dos import RegistroReproduccion
from seguimiento_lidar import SeguimientoLidar, PUERTA_MM as PUERTA_HEREDADA
import time
...
# ESTADOS EN LOS QUE SE PREPARA EL PILAR SIGUIENTE.
#
# APERTURA queda fuera a proposito: ahi el arbitraje del pilar ACTUAL
# baja a "no chocar" porque su lado todavia no se puede cumplir, asi que
# desviar el comando por el siguiente puede estropear el paso legal del
# que se esta rodeando. CRUCERO tambien queda fuera: sin pilar actual, la
# captura y la FSM ya se ocupan del que viene.
#
# RECUPERACION si entra, y es el cambio que importa. Reconstruida la
# corrida 004228 en marco mundo, entre t=3,1 y t=6,0 existia una banda de
# comandos sostenidos (-12 a -20 grados) que dejaba verde y rojo de su
# lado con mas de 40mm de holgura. La consigna salio de esa banda en
# t=3,8 -- al quedar el verde al costado, la regla dejo de mandar y volvio
# el seguimiento de pared -- y 1,9s despues, cuando el rojo paso a ser el
# objetivo, ya no quedaba ningun arco legal: el mejor comando pasaba de
# +163mm de holgura a -2mm en un segundo. RECUPERACION es la mitad de esa
# ventana.
ESTADOS_PREPARACION = ("PASO_LATERAL", "SALIDA_PILAR", "RECUPERACION")

# EL HUECO DEL RELEVO: entre fichar un pilar y abrirse, nadie mira su lado.
#
# CONFIRMACION y COMPROMISO arbitran con pilar=None, y APERTURA lo hace
# con criterio="choque". O sea que desde que el pilar se adopta hasta que
# la apertura toma el mando, lo unico que se comprueba es no tocarlo:
# curvar hacia el lado prohibido esta permitido.
#
# Medido en la corrida 015816. El rojo se adopta en t=6,34 con el arco
# -15 cruzandolo a su lado con 140mm de margen. La consigna fue +7,5,
# +20, +20, +12,5 -- seguimiento de pared, que no sabe del rojo. El servo,
# a 12 grados por ciclo, tardo 0,6s en volver a -20, y para entonces el
# margen habia caido de +97mm a +40mm. En t=7,84 la restriccion de choque
# ya retiraba -20 por falta de sitio y el rojo quedo a la derecha: ilegal.
# El arco legal existio todo ese rato; se gasto girando al reves.
ESTADOS_CRUCE_ACTUAL = ("CONFIRMACION", "COMPROMISO")
...
class Navegador:
    def __init__(self, control_sector):
        ...
        self._dos_modo = os.environ.get("WRO_DOS_PILARES", "0")
        self._dos_memoria = os.environ.get("WRO_DOS_MEMORIA") == "1"
        self._dos = SiguientePilar()
        self._dos_seg = SeguimientoLidar()
        self._dos_preparacion = None
        self._dos_cruce = None
        self._dos_heading = None
        self._dos_cuadro = None
        self._dos_csv = None
        self._dos_fichero = None
        self._dos_filas = 0
        if self._dos_modo in ("sombra", "activo"):
            os.makedirs("logs", exist_ok=True)
            self._dos_fichero = open("logs/dos_%s.csv" % time.strftime("%Y%m%d_%H%M%S"), "w", newline="")
            atexit.register(self._dos_fichero.close)
            self._dos_replay = RegistroReproduccion(self._dos_fichero.name + ".jsonl")
            self._dos_csv = csv.writer(self._dos_fichero)
            self._dos_csv.writerow(["t_unix", "t_mono", "estado", "actual_id", "actual_color",
                                    "actual_x", "actual_y", "secuencia", "siguiente_color",
                                    "siguiente_x", "siguiente_y", "confirmaciones", "motivo",
                                    "cmd_base", "cmd_propuesto", "aplicado", "fuente", "en_seccion", "siguiente_sigma", "edad_siguiente",
                                    "seguros", "compatibles", "alcanzables", "anticipacion_habilitada",
                                    "cerca_salida", "preferir_camara", "seguridad_arbitraje",
                                    "solo_obstaculo", "velocidad_mm_s", "cmd_deseado", "cmd_anterior",
                                    "seg_color", "seg_x", "seg_y", "seg_sigma", "seg_fuente",
                                    "seg_motivo", "seg_racha", "seg_manda", "seg_edad_barrido",
                                    "cruce_propuesto", "cruce_aplicado"])
    ...
    def procesar(self, med, color_cam, heading, ahora=None, cx_cam=None,
                 color_piso=None, cuadro_red=None):
    ...
    def _ciclo_carrera(self, med, color_cam, heading, ahora, dt, cx_cam=None,
        ...
        if self._dos_modo in ("sombra", "activo"):
            giro = 0.0 if self._dos_heading is None else gev.normalizar_180(heading - self._dos_heading)
            self._dos_heading = heading
            bins_dos = bins_de_clusters(med.clusters_estrechos)
            candidatos_dos = self._candidatos_pilar(med)
            ahora_dos = time.monotonic()
            barrido = (med.secuencia, med.mono)
            seccion_dos = lambda x, y: self._en_mi_seccion(
                med, x, y, math.degrees(math.atan2(x, y)), bins_dos)
            self._dos_replay.guardar(self._dos_cuadro, candidatos_dos, self.tracker,
                giro, avance_mm, ahora_dos, seccion_dos, self.estado, barrido)
            self._dos.actualizar(self._dos_cuadro, candidatos_dos, self.tracker,
                giro, avance_mm, ahora_dos, seccion_dos)
            # El seguimiento vive del cluster, no de la caja: mantiene medida
            # la posicion del siguiente durante los ciclos en que la camara
            # ya no lo encuadra, que son justo los de la salida del actual.
            self._dos_seg.actualizar(self._dos, candidatos_dos, self.tracker,
                giro, avance_mm, ahora_dos, seccion_dos, barrido)
    ...
    def _intentar_capturar(self, med, color_cam, heading, cx_cam, ahora):
        ...
        if color_cam is None:
            # SIN CAMARA, PERO CON IDENTIDAD. Un pilar puede salir del
            # encuadre al empezar la maniobra del anterior y volver a
            # entrar en el sector solo unas decimas. Corrida 040743: el
            # rojo entra en t=4,51 y t=4,62 y ya no vuelve; sin poder
            # ficharlo, el robot siguio en CRUCERO y lo dejo atras por
            # el lado prohibido.
            #
            # El color no se inventa: lo trae una identidad que camara y
            # LiDAR confirmaron juntas y que el cluster ha mantenido sin
            # romperse. Y solo se ficha si un UNICO cluster cae en la
            # puerta de esa identidad -- "el mas cercano al robot"
            # podria ser un trozo de muro.
            if self._dos_modo != "activo" or self.tracker.activo:
                return
            heredada = self._dos_seg.identidad_para_captura(ahora)
            if heredada is None:
                return
            color_cam = heredada[0]
        ...
        if heredada is not None:
            cerca = [c for c in candidatos
                     if math.hypot(c[0] - heredada[1], c[1] - heredada[2])
                     <= PUERTA_HEREDADA]
            if len(cerca) == 1:
                el = cerca[0]
    ...
    def _arbitrar(self, deseado, med, velocidad, pilar=None, holgura=0.0,
        ...
        if self._dos_modo in ("sombra", "activo"):
            base = elegido
            ahora_dos = time.monotonic()
            # El pilar actual ya tiene que estar al costado: preparar el
            # siguiente no puede adelantarse al paso del que se rodea.
            cerca_salida = (not self.tracker.activo
                            or self.tracker.xy_eje()[1] < gev.X_MORRO + 100.0)
            habilitado = (self.estado in ESTADOS_PREPARACION
                          and cerca_salida and not preferir_camara
                          and self.seguridad == "LIBRE" and not self._solo_obstaculo)
            # Solo comandos alcanzables sin que el limitador los cambie despues.
            alcanzables = [c for c in candidatos
                           if abs(c - self._ultimo_angulo) <= MAX_DELTA_ANGULO]
            # El seguimiento por LiDAR manda cuando tiene medida propia de
            # este barrido; si no, decide el observador con su caja.
            seg_manda = self._dos_seg.autoriza_control(ahora_dos)
            estimador = self._dos_seg if seg_manda else self._dos
            propuesta = base
            if habilitado and estimador.color is not None:
                propuesta = estimador.proponer(
                    alcanzables, base, _vel_mm_s(velocidad), HOLGURA_BASE, ahora_dos,
                    lado_obligatorio(estimador.color), self._dos_preparacion)
            autoriza = seg_manda or self._dos.autoriza_control(
                ahora=ahora_dos, memoria=self._dos_memoria)
            # La eleccion se conserva de un ciclo al siguiente: el limitador
            # del servo mueve 12 grados por ciclo y cambiar de intencion cada
            # ciclo aplica una consigna que no es ninguna de las dos.
            self._dos_preparacion = propuesta if (autoriza and propuesta != base) else None
            aplicado = self._dos_modo == "activo" and propuesta != base and autoriza
            if aplicado:
                elegido = propuesta
            # El pilar ACTUAL, mientras ninguna fase comprueba su lado.
            # Misma pregunta que la preparacion y la misma funcion, pero
            # sobre el pilar fichado y con la posicion del tracker, que es
            # la que el arbitraje ya usa para decidir choque.
            #
            # No relaja nada: se elige dentro de `alcanzables`, que ya paso
            # paredes, y `cruza_al_lado` exige que el poste quede FUERA del
            # anillo barrido por RADIO_POSTE + holgura, que es mas estricto
            # que la comprobacion de choque de estas fases. Los dos juegos
            # de estados son disjuntos, asi que nunca compiten por `elegido`.
            cruce = base
            if (self.estado in ESTADOS_CRUCE_ACTUAL and self.tracker.activo
                    and not preferir_camara and self.seguridad == "LIBRE"):
                x_a, y_a = self.tracker.xy_eje()
                cruce = preparar_paso(alcanzables, base, x_a, y_a,
                                      self.tracker.s_lado, HOLGURA_BASE,
                                      self._dos_cruce, sesgo=None,
                                      mejora=MEJORA_MINIMA_MM)
            self._dos_cruce = cruce if cruce != base else None
            cruce_aplicado = self._dos_modo == "activo" and cruce != base
            if cruce_aplicado:
                elegido = cruce
            self._dos_csv.writerow([time.time(), ahora_dos, self.estado,
                self.tracker.id, self.tracker.color, self.tracker.x, self.tracker.y,
                self._dos.secuencia, self._dos.color, self._dos.x, self._dos.y,
                self._dos.confirmaciones, self._dos.motivo, base, propuesta, int(aplicado), self._dos.fuente, self._dos.en_seccion, self._dos.sigma,
                None if self._dos.t is None else ahora_dos - self._dos.t,
                seguros, candidatos, alcanzables, habilitado, cerca_salida, preferir_camara,
                self.seguridad, self._solo_obstaculo, _vel_mm_s(velocidad), deseado, self._ultimo_angulo,
                self._dos_seg.color, self._dos_seg.x, self._dos_seg.y, self._dos_seg.sigma,
                self._dos_seg.fuente, self._dos_seg.motivo, self._dos_seg.racha, int(seg_manda),
                None if self._dos_seg.t_lidar is None else ahora_dos - self._dos_seg.t_lidar,
                cruce, int(cruce_aplicado)])
            self._dos_filas += 1
            if self._dos_filas % 10 == 0:
                self._dos_fichero.flush()
