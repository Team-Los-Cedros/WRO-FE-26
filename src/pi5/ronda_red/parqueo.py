...
LIDAR_A_FLANCO_CERCA = 44.0
LIDAR_A_FLANCO_LEJOS = ANCHO_REAL - LIDAR_A_FLANCO_CERCA    # 94 mm
...
MARGEN_RECORRIDO_ATRAS = 25.0    # mm que NO se gastan del hueco medido
...
MIN_COSTADO = LIDAR_A_FLANCO_CERCA + 22.0      # 66 mm de lectura
...
def alinear(sen, rumbo_meta, seco):
    """Enderezarse con el carril. Termina POR RUMBO, no por reloj: es el
    mismo criterio que rompio el ciclo limite de las esquinas."""
    print("[ALINEAR] meta %.1f grados (el rumbo con el que estaba aparcado)"
          % rumbo_meta)
    if seco:
        print("          modo seco: no se manda nada")
        return True
    ...
    while time.time() - t0 < TIMEOUT_ALINEAR:
        err = rumbo_meta - sen.rumbo()
        if abs(err) < TOL_ALINEADO:
            sen.parar()
            print("          a rumbo: %.1f grados" % sen.rumbo())
            return True
        # SEGURIDAD, que aqui no habia ninguna: enderezar son ~365 mm de
        # recorrido dentro de un carril de 1000, y el robot sale de la
        # plaza cruzado. Sin esto el enderezado se come la pared de
        # enfrente o la isla.
        d = sen.rango(0.0, SECTOR_AVANCE)
        if d is not None and d < MARGEN_FRENTE + 60.0:
            sen.parar()
            print("          pared a %.0f mm: se corta el enderezado en %.1f grados"
                  % (d, sen.rumbo()))
            return False
        cmd = err * KP_ALINEAR
        ...
        time.sleep(1.0 / HZ)
    sen.parar()
    print("          tiempo maximo, error %.1f grados" % (rumbo_meta - sen.rumbo()))
    return False
...
def main():
    ...
    print()
    if ok or seco:
        alinear(sen, rumbo0, seco)
        print()
        print("[LISTO] fuera de la plaza y a rumbo. Aqui entra la FSM de carrera.")
    else:
