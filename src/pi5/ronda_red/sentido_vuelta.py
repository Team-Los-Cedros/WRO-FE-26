...
class SentidoPorGeometria:
    ...
    def sembrar(self, sentido, origen="la salida del estacionamiento"):
        """Sentido que YA se sabia antes de arrancar la carrera.

        Traido tal cual de ronda_curvas, donde lo usa la salida del
        estacionamiento: `salida_manual.py` mira la linea de esquina con el
        robot todavia aparcado y deja el sentido escrito. Sin esto la
        carrera volvia a deducirlo desde cero.

        Se siembra como PISTA Y NO COMO CERTEZA: `por_lineas` se queda en
        False a proposito. La carrera arranca sabiendo el sentido desde el
        primer ciclo, pero cualquier linea que se vea despues lo corrige sin
        pelea, que es justo lo que no se podria hacer si se fijara aqui.
        """
        if sentido not in (-1, 1) or self.sentido != 0:
            return False
        self.sentido = sentido
        self.evidencia = 0.0
        print("[SENTIDO] %s de entrada, visto en %s (revisable)"
              % ("HORARIA" if sentido < 0 else "ANTIHORARIA", origen))
        return True
