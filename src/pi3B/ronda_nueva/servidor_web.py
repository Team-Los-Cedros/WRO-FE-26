...
async function tic(){
  try{
    ...
    pinta('marcha', [
      ...
      ['rumbo', num(d.heading,1)+'&deg;', ''],
      ['esquinas', num(d.esquinas), ''],
      ['watchdog', wd, wd==='OK'?'ok':'mal'],
      ['piso', d.color_piso || '--',
        (d.color_piso==='SIN_SENSOR')?'mal':''],
    ]);
    pinta('distancias', [
      ['frontal', num(d.frontal), (d.frontal<150)?'mal':(d.frontal<300?'avi':'')],
      ['izquierda', num(d.izquierda), ''],
      ['derecha', num(d.derecha), ''],
      ['trasera', num(d.trasera), ''],
      ['ultrasonido', num(d.ultrasonido_mm), ''],
      ['calidad pared', num(d.calidad_pared,2), ''],
    ]);
    pinta('extra', [
      ['hueco conf.', num(d.hueco_confianza,2),
        (d.hueco_confianza>0)?'ok':''],
      ['hueco sep.', num(d.hueco_separacion_mm), ''],
      ['tracks', num(d.tracks), ''],
      ['pilar activo', d.track_activo_color || '--', ''],
