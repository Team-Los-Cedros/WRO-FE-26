# ronda_cerrada — el cerebro de la 3B, portado a la Pi 5

> **Esto es un PORT.** El contenido a partir de "ronda_nueva" es la bitácora
> original tal y como se escribió en la Raspberry Pi 3B, y se conserva entera
> porque es donde están los porqués. Lo que sigue es solo lo que cambió al
> traerlo, y por qué no se podía copiar tal cual.
>
> Convive con `ronda_nueva/` (el cerebro de mapa de la Pi 5), no lo sustituye.
> Los dos se despliegan juntos y se lanzan por separado:
>
> ```bash
> python3 -m ronda_cerrada.ronda_nueva --validar-config
> python3 -m ronda_nueva.ronda_nueva  --validar-config
> ```

## Qué cambió al portarlo (2026-09-05)

**Origen:** la copia de `src/pi3B/ronda_nueva` del zip del 2 de septiembre.
Ojo: esa copia es anterior a cinco commits de la rama, entre ellos
`34c87bf` *"el sobrepaso sale por geometría, no por cronómetro"*, que es el
arreglo del pilar rojo.

Ningún cambio es cosmético; los cuatro salen de medir este robot.

### 1. La máscara del mástil, remedida

La configuración heredada traía `blind_sectors_deg = [[163, 195]]`, que es del
montaje de la 3B. Medido tres veces en la Pi 5 con
`herramientas/diag_mastil.py`, la estructura fija ocupa **141..212 grados** a
35-68 mm, con eco en el 100 % de los barridos. Pasa a `[[140, 213]]`.

**La máscara es del MONTAJE, no del robot: se remide cada vez que se toca la
cámara.**

### 2. El soporte de la cámara contamina el sector derecho

Hay una segunda estructura fija en **28..54 grados a 142-172 mm**. No va en
`blind_sectors_deg`: ese bloque describe la cuña *trasera* que el compensador
de oclusión sabe proyectar, y `configuracion_desde_mapa` admite **una sola** —
se niega antes que aplicar una geometría que no sabe compensar, y hace bien.

Además los sectores laterales no consultan esa máscara, y el filtro de autoeco
(`self_echo_max_mm` = 150 mm) no cubre un eco a 142-172 mm: lo atraviesa. Sin
tocarlo, la distancia derecha se quedaría clavada en ~142 mm y el centrado
entre paredes dejaría de funcionar.

Se resuelve estrechando el sector: `right_sector_deg` pasa de `[45, 135]` a
`[56, 135]`. Se pierden 11 de 90 grados; el diagnóstico ve 133 puntos de pared
derecha en ese sector, así que sobran.

### 3. Los hombros traseros estaban ciegos al 100 %

`rear_shoulder_offset_deg` era `[18, 35]`, o sea ventanas en 145-162 y 198-215.
Con el mástil en 141..212, **las dos caen enteras dentro de la estructura**: el
robot no podía medir hacia atrás en absoluto. Pasan a `[40, 60]` (ventanas
120-140 y 220-240), que es lo primero que este montaje deja ver.

Tiene un coste que hay que saber: la trasera se obtiene proyectando rayos que
ahora salen a 40-60 grados del eje, no a 18-35. Una pared a 620 mm de rayo son
620·cos(50°) = **399 mm de eje** en vez de los 557 de antes. Es geometría
correcta, pero más sensible a que la pared no esté plana y perpendicular.
**Para el parqueo, el dato bueno de atrás es el ultrasonido**, que este
paquete ya sabe fusionar (`parking.ultrasound_rear_enabled`).

### 4. `turn_direction: AUTO` no funciona en este robot

AUTO resuelve el sentido leyendo la línea de piso con el TCS3472, y en la Pi 5
la Pico responde `COLOR:SIN_SENSOR`: **el sensor no está montado**. Con AUTO la
FSM se quedaría en `WAIT_DIRECTION` hasta agotar `direction_timeout_s`. Pasa a
`RIGHT`; hay que ponerlo a mano según el sentido del sorteo, o engancharlo a la
detección de líneas por cámara que sí tiene `ronda_nueva/vision_pista.py`.

## Lo que NO se ha verificado

- **La extrínseca cámara-LiDAR es la de la 3B** (`forward_from_lidar_mm`
  −99,76 y compañía) y el montaje ha cambiado —el mástil se movió 18 grados—.
  La marca `camera_lidar_extrinsics_ready` viene en `true` heredada, y eso es
  optimista. Degrada la asociación de color con los pilares.
- **Nunca ha rodado con motores en esta placa.** `motion_enabled` sigue en
  `false` y quedan tres calibraciones pendientes, igual que en la 3B.

## Pruebas

145 pruebas offline, todas en verde:

```bash
python -m unittest discover -s src/pi5/ronda_cerrada/tests -t . 
```

Cuatro tuvieron que actualizarse porque fijaban a mano un valor de calibración
del JSON de despliegue en vez de la propiedad que querían comprobar. Es el
mismo error que ya apareció con `parking_side`: una prueba no debe romperse
porque la pista cambie de sentido o el mástil se mueva.
