...
from src.pi5.comun.lidar_geometria import Medicion, construir_perfil_360
from src.pi5.ronda_cerrada.config import cargar_configuracion
from src.pi5.ronda_cerrada.percepcion_lidar import PercepcionLidar
...
class PercepcionLidarTests(unittest.TestCase):
    ...
    def test_mastil_no_contamina_reversa_y_sin_eco_no_es_via_libre(self):
        # El mastil ocupa 141..212 en este montaje (medido el 2026-09-05), asi
        # que los ecos de muro tienen que caer FUERA de ese arco o no serian
        # muro sino la propia estructura: 130-131 sobreviven por el sector
        # trasero derecho (90-162) y 220-221 por el izquierdo (198-270).
        scan = [(float(a), 92.0) for a in range(141, 213)]
        scan += [(130.0, 620.0), (131.0, 625.0), (220.0, 640.0), (221.0, 635.0)]
        ...
        # Lo que fija esta prueba es que la trasera NO sea el eco del propio
        # mastil (92 mm), no un numero concreto. El valor sale de proyectar
        # los rayos de los hombros sobre el eje trasero, asi que depende de
        # donde esten esos hombros: con los 40-60 grados que impone este
        # montaje, 620 mm de rayo son 620*cos(50) = 399 mm de eje. Con los
        # 18-35 grados de la 3B eran 557. Por eso aqui no hay umbral magico.
        self.assertGreater(corredor.trasera_mm, 300.0)
