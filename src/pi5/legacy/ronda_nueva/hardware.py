...
class EnlacePicoNuevo:
    ...
    def fijar_cero(self) -> None:
        with self._lock:
            ...
            self._deriva_deg_s = 0.0
            self._t_deriva = time.monotonic()

    def calibrar_deriva(self, segundos: float = 2.5) -> float:
        """Mide y compensa la deriva del giroscopio con el robot QUIETO.

        POR QUE HACE FALTA
        El firmware de la Pico ya promedia 100 muestras al arrancar para quitar
        el sesgo, pero ese bucle se traga sus excepciones: si el I2C aun no
        responde -- lo normal justo despues de alimentar la placa -- el offset
        se queda en cero y el sesgo crudo entra entero en la integracion.
        Medido el 04-09 en el robot quieto: **+20,9 grados/s**, perfectamente
        lineal.  Un giro de esquina dura unos dos segundos, asi que la deriva
        sola metia 42 grados de error en cada esquina.

        Aqui se mide otra vez desde la Pi, ya con todo encendido y estabilizado,
        y se resta linealmente.  Es una red de seguridad barata: si el firmware
        hizo bien su trabajo, esto mide casi cero y no cambia nada.

        Devuelve la deriva en grados por segundo.  El robot NO se puede mover
        durante la medida.
        """

        self.fijar_cero()
        inicio = time.monotonic()
        time.sleep(max(0.5, float(segundos)))
        with self._lock:
            cero = self._cero_yaw
            crudo = self._yaw_crudo
        transcurrido = max(1e-3, time.monotonic() - inicio)
        deriva = 0.0 if cero is None else (crudo - cero) / transcurrido
        with self._lock:
            self._deriva_deg_s = deriva
            self._cero_yaw = crudo
            self._t_deriva = time.monotonic()
        return deriva

    @property
    def deriva_deg_s(self) -> float:
        with self._lock:
            return self._deriva_deg_s

    def heading(self) -> float:
        """Rumbo acumulado desde el cero, ya sin la deriva medida."""

        with self._lock:
            cero = self._cero_yaw
            if cero is None:
                return 0.0
            bruto = self._yaw_crudo - cero
            if not self._deriva_deg_s:
                return bruto
            return bruto - self._deriva_deg_s * (time.monotonic() - self._t_deriva)
