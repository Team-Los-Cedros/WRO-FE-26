...
class ControlEstacionamiento:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.profundidad_bahia_mm = float(parqueo.get("bay_depth_mm", 200.0))
    ...
    def _mando(self, hacia_bahia: bool, reversa: bool) -> float:
        ...
        # La marcha atras invierte el yaw, pero el desplazamiento lateral
        # del ARCO conserva el lado del volante (depende de 1-cos(theta)).
        hacia_izquierda = self._lado < 0
        if not hacia_bahia:
    ...
    def _alinear(
        ...
        if (abs(error) <= 35.0 and lateral is not None
                and abs(lateral - self.lateral_objetivo_mm) <= 30.0
                and paralelo is not None and abs(paralelo) <= self.tolerancia_paralelo_deg):
    ...
    def _dentro(self, lateral: Optional[float], paralelo: Optional[float]) -> bool:
        if lateral is None or paralelo is None:
            return False
        if abs(paralelo) > self.tolerancia_paralelo_deg:
            return False
        # Comprobar proyeccion de TODO el cuerpo, no solo el centro LiDAR.
        a = abs(math.radians(paralelo))
        ancho_proyectado = self.ancho_robot_mm * math.cos(a) + self.largo_robot_mm * math.sin(a)
        largo_proyectado = self.largo_robot_mm * math.cos(a) + self.ancho_robot_mm * math.sin(a)
        margen = 8.0
        return (lateral >= ancho_proyectado / 2.0 + margen
                and lateral + ancho_proyectado / 2.0 <= self.profundidad_bahia_mm - margen
                and largo_proyectado + 2.0 * margen <= self.largo_bahia_mm)
    ...
    def _verificar(
        ...
        frontal = paredes.frontal_min_mm
        bien = (
            self._dentro(lateral, paralelo)
            and trasera is not None
            and 15.0 <= trasera <= self.largo_bahia_mm - self.largo_robot_mm - 15.0
            and math.isfinite(frontal) and frontal >= self.holgura_frontal_min_mm * 0.5
        )
