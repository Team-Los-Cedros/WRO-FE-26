...
from src.pi5.ronda_cerrada.config import (
    ErrorConfiguracion,
...
from src.pi5.ronda_cerrada.hardware import EnlacePicoNuevo
...
class ConfiguracionTests(unittest.TestCase):
    def test_config_porta_las_mediciones_del_mastil(self):
        ...
        # La mascara es del MONTAJE, no del robot, y este cerebro corre ahora
        # sobre la Pi 5, cuyo mastil esta en otro sitio. Medido tres veces con
        # `herramientas/diag_mastil.py` el 2026-09-05: la estructura fija
        # ocupa 141..212 grados a 35-68 mm, con eco en el 100 % de los
        # barridos que miraron esos grados. La mascara heredada de la 3B
        # (163-195) dejaba 141..162 y 196..212 sin tapar, y por ahi entraba
        # el propio mastil como si fuera un muro a 48 mm.
        self.assertEqual(config["lidar"]["blind_sectors_deg"], [[140.0, 213.0]])
        # El soporte de la camara devuelve otro eco fijo en 28..54 grados a
        # 142-172 mm. NO va en blind_sectors_deg -- ese bloque describe la
        # cuña TRASERA que el compensador sabe proyectar, y admite una sola --
        # sino que se excluye estrechando el sector lateral derecho.
